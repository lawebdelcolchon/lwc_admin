// src/features/user/ForgotPassword.js
...
function ForgotPassword() {
  ...
  const submitForm = async (e) => {
    ...
    if (userObj.emailId.trim() === '') return setErrorMessage('Email Id is required! (use any value)');
      ...
      try {
        setLoading(true);
        await dispatch(forgotPassword(userObj.emailId));
        setLoading(false);
        setLinkSent(true);
      } catch (error) {
        setLoading(false);
        setErrorMessage(error.message); // Maneja cualquier error que pueda ocurrir
      }
    }
  };
