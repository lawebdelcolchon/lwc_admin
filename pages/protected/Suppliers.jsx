...
function InternalPage(){
    ...
    useEffect(() => {
        dispatch(setPageTitle({ title : "Proveedores"}))
    }, [])

    const icons = [
        {
            icon: HiOutlineLockOpen,
            message: 'Bloquear Proveedor',
            switch_icon: HiOutlineLockClosed,
            switch_message: 'Desbloquear Proveedor',
            field: "enabled",
            action: 'update', // Acción asociada
            updateValue: (currentValue) => currentValue === 0 ? 1 : 0 // Alternar entre 0 y 1
        },
        { icon: TrashIcon, message: 'Eliminar proveedor' },
    ];
...
//      { type: "file", name: "name", labelTitle: "Archivo", value: "",  position: "left", size: "sm", border: "bordered"},
    ...
    const codeItems = [
      { 
        type: "datatable",
        entityName: "suppliers_codes",
        columns: ["name", "description", "code_shipping", "code_control", "code_franchise", "active"],
        titles: ["Transportista", "Descripción", "Cod. Envío", "Cod. Control", "Cod. Franquicia", "Estatus"],
        query: [{
          fields: 'gt_carriers.name, description, code_shipping, code_control, code_franchise, image_url, html_url, active ',
          pivotTable: 'gt_suppliers_codes',
          joinTable: 'gt_carriers',
          joinCondition: 'gt_suppliers_codes.id_carrier = gt_carriers.id',
          joinType: 'LEFT',
          whereCondition: 'WHERE id_supplier = ',
    ...
    return(
        <EntityList 
            ...
            dbName="lawebes"
            entityTitles={["Id", "Proveedores", "Correo", "Teléfono", "Fecha de registro", "Estado"] }
            entityMinTitles={["Id", "Proveedores"] }
            fields="id, name, email, phone, date_supplier, enabled" 
            fieldsMin="id, name" 
            interval={50} 
            sort="name"
            sortOrder="asc"
            ...
            mergedColumns={[
                ...
                { fields: ["date_supplier"], title: "Fecha de Registro" },
            ...
            panels = {[
                ...
                {fields: codeItems, title: "Códigos"}
            ]}
        />
    )
}
